...
(function () {
  ...
  const fresh = () => ({ profiles: [{ id: 'p1', name: '', avatar: '🦁' }], active: 'p1', prog: {}, bonus: null, usage: {},
    settings: { limitMin: 30, reciter: 'Husary_Muallim_128kbps', repeat: 1, echo: false, sound: true, order: 'abc', enOrder: 'abc', auto: true } });
  function load() {
    try {
  ...
  const praise = () => 'ui/' + (rnd() < 0.5 ? 'great' : 'super');
  ...
  const AREAS = { literacy: '📖 القراءة', math: '🔢 الرياضيات', thinking: '🧠 التفكير' };
  ...
  function parentsPanel() {
    ...
    view.innerHTML = `
      ...
      <div class="card"><h3>📚 منهجنا</h3>
        <p class="note">القراءة: القاعدة النورانية (الحرف ← الحركة ← المقطع ← الكلمة) والحروف المتشابهة معاً · الإنجليزي: Phonics · الأرقام: الإطار العشري وإدراك الكمية بلا عدّ · التعلّم التكيّفي: «رحلة اليوم» تُبنى لكل طفل من أضعف ما عنده.</p>
